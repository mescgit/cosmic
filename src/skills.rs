...
#[derive(Component)] pub struct ActiveSkillAoEEffect { pub _skill_id: SkillId, pub actual_damage_per_tick: i32, pub actual_radius_sq: f32, pub tick_timer: Timer, pub lifetime_timer: Timer, pub already_hit_this_tick: Vec<Entity>, }
...
impl Plugin for SkillsPlugin {
    fn build(&self, app: &mut App) {
        app .register_type::<SkillId>() .register_type::<SkillEffectType>() .register_type::<SkillDefinition>() .register_type::<ActiveSkillInstance>() .register_type::<SkillLibrary>()
            ...
            .add_systems(Update, ( active_skill_cooldown_recharge_system, survivor_skill_input_system, skill_projectile_lifetime_system, skill_projectile_collision_system, active_skill_aoe_system, survivor_buff_management_system, freezing_nova_effect_damage_system, 
    ...
    if let Ok((player_entity, mut player, player_transform)) = player_query.get_single_mut() {
        ...
        if let Some(idx) = skill_to_trigger_idx { if idx >= player.equipped_skills.len() { return; } let current_aim_direction = player.aim_direction; let skill_instance_snapshot = player.equipped_skills[idx].clone();
            if skill_instance_snapshot.is_ready() { if let Some(skill_def) = skill_library.get_skill_definition(skill_instance_snapshot.definition_id) {
                ...
                let mut aoe_radius = 0.0; 
                let mut sentry_damage_val = 0; 
                let mut sentry_radius_val = 0.0; 
                let mut nova_damage_val = 0; 
                let mut nova_radius_val = 0.0;

                match &skill_def.effect { 
                    SkillEffectType::Projectile { base_damage, piercing: base_piercing, .. } => { 
                        projectile_damage = *base_damage + skill_instance_snapshot.flat_damage_bonus; 
                        projectile_piercing = *base_piercing; 
                    } 
                    SkillEffectType::AreaOfEffect { base_damage_per_tick, base_radius, .. } => { 
                        ...
                        aoe_radius = *base_radius * skill_instance_snapshot.aoe_radius_multiplier; 
                    }, 
                    SkillEffectType::SummonSentry { sentry_damage_per_tick: sdpt, sentry_radius: sr, ..} => { 
                        ...
                        sentry_radius_val = *sr * skill_instance_snapshot.aoe_radius_multiplier; 
                    } 
                    SkillEffectType::FreezingNova { damage, radius, .. } => { 
                        nova_damage_val = *damage + skill_instance_snapshot.flat_damage_bonus; 
                        nova_radius_val = *radius * skill_instance_snapshot.aoe_radius_multiplier; 
                    } 
                    _ => {} 
                }
                        ...
                        if let Some(glyph_def) = glyph_library.get_glyph_definition(*glyph_id) { 
                            match &glyph_def.effect { 
                                GlyphEffectType::AddedChaosDamageToProjectile { damage_amount } => { 
                                    if matches!(skill_def.effect, SkillEffectType::Projectile {..}) { 
                                        projectile_damage += *damage_amount; 
                                    } 
                                } 
                                GlyphEffectType::IncreasedAoEDamage { percent_increase } => { 
                                    if matches!(skill_def.effect, SkillEffectType::AreaOfEffect {..}) { 
                                        aoe_damage_per_tick = (aoe_damage_per_tick as f32 * (1.0 + percent_increase)).round() as i32; 
                                    } 
                                    if matches!(skill_def.effect, SkillEffectType::SummonSentry {..}) { 
                                        sentry_damage_val = (sentry_damage_val as f32 * (1.0 + percent_increase)).round() as i32; 
                                    } 
                                    if matches!(skill_def.effect, SkillEffectType::FreezingNova {..}) { 
                                        nova_damage_val = (nova_damage_val as f32 * (1.0 + percent_increase)).round() as i32; 
                                    } 
                                } 
                                GlyphEffectType::ProjectileChain { bounces } => { 
                                    if matches!(skill_def.effect, SkillEffectType::Projectile {..}) { 
                                        projectile_bounces += *bounces; 
                                    } 
                                } 
                            } 
                        } 
                ...
                match &skill_def.effect {
                    SkillEffectType::Projectile { speed, size, color, lifetime_secs, .. } => { 
                        if current_aim_direction != Vec2::ZERO { 
                            ...
                            commands.spawn(( 
                                SpriteBundle { 
                                    ...
                                    transform: Transform::from_translation(projectile_spawn_position) .with_rotation(Quat::from_rotation_z(current_aim_direction.y.atan2(current_aim_direction.x))), ..default() 
                                }, 
                                SkillProjectile { 
                                    skill_id: skill_def.id, 
                                    piercing_left: projectile_piercing, 
                    ...
                    SkillEffectType::AreaOfEffect { base_damage_per_tick: _, tick_interval_secs, duration_secs, color: aoe_color, .. } => { 
                        if skill_def.id == SkillId(2) { 
                            ...
                            for i in 0..num_projectiles {
                                ...
                                commands.spawn((
                                    ...
                                    Damage(aoe_damage_per_tick), 
                                    Lifetime { timer: Timer::from_seconds(0.4, TimerMode::Once) }, 
                            ...
                            commands.spawn(( 
                                SpriteBundle { 
                                    texture: asset_server.load("sprites/mind_shatter_effect_placeholder.png"), 
                                    sprite: Sprite { custom_size: Some(Vec2::splat(aoe_radius * 2.0)), color: *aoe_color, ..default()}, 
                                    transform: Transform::from_translation(aoe_spawn_position.truncate().extend(0.2)), ..default() 
                                }, 
                                ActiveSkillAoEEffect { 
                                    _skill_id: skill_def.id, 
                                    actual_damage_per_tick: aoe_damage_per_tick, 
                                    actual_radius_sq: aoe_radius.powi(2), 
                                ...
                                Name::new(format!("SkillAoE_{}", skill_def.name)), 
                            )); 
                            effect_was_triggered = true;
                        }
                    }
                    SkillEffectType::SurvivorBuff { speed_multiplier_bonus, fire_rate_multiplier_bonus, duration_secs } => { 
                        commands.entity(player_entity).insert(SurvivorBuffEffect { 
                            speed_multiplier_bonus: *speed_multiplier_bonus, 
                            fire_rate_multiplier_bonus: *fire_rate_multiplier_bonus, 
                    ...
                    SkillEffectType::SummonSentry { sentry_tick_interval_secs, sentry_duration_secs, sentry_color, .. } => { 
                        ...
                        commands.spawn(( 
                            SpriteBundle { 
                                texture: asset_server.load("sprites/psychic_sentry_placeholder.png"), 
                                sprite: Sprite { custom_size: Some(Vec2::splat(sentry_radius_val * 0.5)), color: *sentry_color, ..default() }, 
                                transform: Transform::from_translation(sentry_spawn_position), ..default() 
                            }, 
                            ActiveSkillAoEEffect { 
                                _skill_id: skill_def.id, 
                                actual_damage_per_tick: sentry_damage_val, 
                                actual_radius_sq: sentry_radius_val.powi(2), 
                                tick_timer: Timer::from_seconds(*sentry_tick_interval_secs, TimerMode::Repeating), 
                                lifetime_timer: Timer::from_seconds(*sentry_duration_secs, TimerMode::Once), 
                                already_hit_this_tick: Vec::new(), 
                            }, 
                            Name::new("PsychicSentry"), 
                        )); 
                        effect_was_triggered = true; 
                    }
                    SkillEffectType::FreezingNova { nova_duration_secs, slow_multiplier, slow_duration_secs, color, .. } => { 
                        let nova_spawn_position = player_transform.translation; 
                        commands.spawn(( 
                            SpriteBundle { 
                                texture: asset_server.load("sprites/frost_nova_placeholder.png"), 
                                sprite: Sprite { custom_size: Some(Vec2::splat(0.1)), color: *color, ..default() }, 
                                transform: Transform::from_translation(nova_spawn_position.truncate().extend(0.25)), ..default() 
                            }, 
                            FreezingNovaEffect { 
                                damage: nova_damage_val, 
                                radius_sq: nova_radius_val.powi(2), 
                                lifetime_timer: Timer::from_seconds(*nova_duration_secs, TimerMode::Once), 
                                slow_multiplier: *slow_multiplier, 
                                slow_duration_secs: *slow_duration_secs, 
                                already_hit_entities: Vec::new(), 
                            }, 
                            Name::new("GlacialNovaEffect"), 
                        )); 
                        effect_was_triggered = true; 
...
fn skill_projectile_collision_system(
    ...
    for (proj_entity, proj_g_transform, proj_damage, mut skill_projectile_data, proj_sprite) in skill_projectile_query.iter_mut() {
        if skill_projectile_data.already_hit_by_this_projectile.len() > (skill_projectile_data.piercing_left + skill_projectile_data.bounces_left + 5) as usize { 
        ...
        for (horror_entity, horror_gtransform, mut horror_health, horror_data) in horror_query.iter_mut() {
            ...
            if proj_pos.distance(horror_pos) < proj_radius + horror_radius {
                ...
                if skill_projectile_data.piercing_left > 0 {
                    ...
                    for (potential_target_entity, potential_target_gtransform, _health, _potential_horror_data) in horror_query.iter() { 
                    ...
                    if let Some((target_entity, _)) = closest_new_target {
                        if let Ok((_t_ent, target_transform, _h, _target_horror_data)) = horror_query.get(target_entity) { 
                                    ...
                                    if let SkillEffectType::Projectile { speed, size, color, lifetime_secs, piercing, .. } = skill_def.effect {
                                        ...
                                        commands.spawn((
                                            ...
                                            SkillProjectile {
                                                ...
                                                bounces_left: skill_projectile_data.bounces_left, 
                                                already_hit_by_this_projectile: vec![target_entity], 
                                            },
