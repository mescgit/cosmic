...
fn horror_spawn_system(
    ...
    let cycle_multiplier = 1.0 + (game_state.cycle_number as f32 - 1.0) * 0.1; // Changed to cycle_number

    let chosen_type = match game_state.cycle_number { // Changed to cycle_number
    ...
    spawn_horror_type(&mut commands, &asset_server, chosen_type, final_spawn_pos, cycle_multiplier, is_elite); // Changed to cycle_multiplier
}
    ...
    for (summoner_transform, mut summoner_behavior) in summoner_query.iter_mut() { 
        ...
        if summoner_behavior.summon_timer.just_finished() && summoner_behavior.active_minion_entities.len() < summoner_behavior.max_minions as usize { 
            for _ in 0..SUMMONER_MINIONS_TO_SPAWN { 
                ...
                let minion_entity = spawn_and_return_horror_entity(&mut commands, &asset_server, HorrorType::CrawlingTorment, minion_spawn_pos, cycle_multiplier); // Changed to cycle_multiplier
                summoner_behavior.active_minion_entities.push(minion_entity); 
            } 
        } 
    } 
}
fn spawn_and_return_horror_entity( commands: &mut Commands, asset_server: &Res<AssetServer>, horror_type: HorrorType, position: Vec3, cycle_multiplier: f32,) -> Entity { // Renamed wave_multiplier to cycle_multiplier
    let stats = HorrorStats::get_for_type(horror_type, cycle_multiplier); // Changed to cycle_multiplier
    commands.spawn(( 
