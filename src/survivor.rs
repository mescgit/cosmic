...
use crate::{
    ...
    ichor_blast::{spawn_ichor_blast}, 
    horror::Horror, 
    // weapons::SwarmOfNightmares, // Already removed, correct
    audio::{PlaySoundEvent, SoundEffect},
    skills::{ActiveSkillInstance, SkillLibrary, SkillId, SurvivorBuffEffect}, 
    items::{ItemId, ItemDrop, ItemLibrary, WeaponId, WeaponLibrary, ItemEffect, RetaliationNovaEffect}, 
    glyphs::GlyphId,
};
...
pub struct Survivor {
    ...
    pub automatic_weapon_damage_bonus: i32, 
    pub automatic_weapon_speed_multiplier: f32, 
    pub automatic_weapon_piercing_bonus: u32, 
    pub xp_gain_multiplier: f32, pub pickup_radius_multiplier: f32, 
    pub additional_automatic_projectiles: u32, 
    ...
    pub equipped_rings: [Option<ItemId>; 1], 
}
...
impl Survivor {
    ...
    pub fn new_with_skills_items_and_weapon(initial_skills: Vec<ActiveSkillInstance>, initial_items: Vec<ItemId>, weapon_id: Option<WeaponId>) -> Self { 
        Self { 
            ...
            automatic_weapon_damage_bonus: 0, 
            automatic_weapon_speed_multiplier: 1.0, 
            automatic_weapon_piercing_bonus: 0, 
            xp_gain_multiplier: 1.0, 
            pickup_radius_multiplier: 1.0, 
            additional_automatic_projectiles: 0, 
...
impl Plugin for SurvivorPlugin { fn build(&self, app: &mut App) { app .add_systems(OnEnter(AppState::InGame), spawn_survivor.run_if(no_survivor_exists)) .add_systems(Update, ( survivor_movement, survivor_aiming, automatic_weapon_casting_system, survivor_health_regeneration_system, survivor_horror_collision_system.before(check_survivor_death_system), survivor_invincibility_system, check_survivor_death_system, survivor_item_drop_collection_system, ).chain().run_if(in_state(AppState::InGame))) .add_systems(OnExit(AppState::InGame), despawn_survivor.run_if(should_despawn_survivor)); } } 

fn spawn_survivor( mut commands: Commands, asset_server: Res<AssetServer>, skill_library: Res<SkillLibrary>, weapon_library: Res<WeaponLibrary>) { 
    ...
    let default_weapon_id = weapon_library.weapons.first().map(|def| def.id); 
    ...
    commands.spawn(( 
        ...
        Survivor::new_with_skills_items_and_weapon(initial_skills, Vec::new(), default_weapon_id), 
        ComponentHealth(INITIAL_SURVIVOR_MAX_HEALTH), 
        Velocity(Vec2::ZERO), 
        SanityStrain::default(), 
        Name::new("Survivor"), 
    )); 
}
...
fn automatic_weapon_casting_system( 
    mut commands: Commands, 
    asset_server: Res<AssetServer>, 
    time: Res<Time>, 
    mut query: Query<(&Transform, &Survivor, &mut SanityStrain, Option<&SurvivorBuffEffect>)>, 
    weapon_library: Res<WeaponLibrary>,
    mut sound_event_writer: EventWriter<PlaySoundEvent>,
) { 
    for (survivor_transform, survivor_stats, mut sanity_strain, buff_effect_opt) in query.iter_mut() { 
        let Some(equipped_weapon_id) = survivor_stats.equipped_weapon_id else { continue; };
        let Some(weapon_def) = weapon_library.get_weapon_definition(equipped_weapon_id) else { continue; };

        let mut current_fire_rate_secs = sanity_strain.base_fire_rate_secs; 
        ...
        if sanity_strain.fire_timer.just_finished() { 
            if survivor_stats.aim_direction != Vec2::ZERO { 
                ...
                let current_damage = weapon_def.base_damage + survivor_stats.automatic_weapon_damage_bonus; 
                let current_speed = weapon_def.base_speed * survivor_stats.automatic_weapon_speed_multiplier; 
                let current_piercing = weapon_def.base_piercing + survivor_stats.automatic_weapon_piercing_bonus;
                let total_projectiles = weapon_def.num_projectiles_at_once + survivor_stats.additional_automatic_projectiles;
                ...
                for i in 0..total_projectiles { 
                    let angle_offset_rad = if total_projectiles > 1 { 
                        let total_spread_angle_rad = (total_projectiles as f32 - 1.0) * weapon_def.projectile_spread_angle_degrees.to_radians(); 
                        let start_angle_rad = base_angle - total_spread_angle_rad / 2.0; 
                        start_angle_rad + (i as f32 * weapon_def.projectile_spread_angle_degrees.to_radians()) 
                    } else { 
                        base_angle 
                    }; 
                    ...
                    spawn_ichor_blast( 
                        ...
                        current_speed, 
                        current_piercing,
                        weapon_def.base_lifetime_secs, 
                        weapon_def.projectile_sprite_path.clone(), 
