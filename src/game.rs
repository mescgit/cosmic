...
use crate::{
    ...
    skills::{ActiveSkillInstance, SkillLibrary, SkillDefinition}, // Removed unused SkillId, SkillProjectile, ActiveSkillAoEEffect
    automatic_projectiles::AutomaticProjectile, 
    glyphs::{GlyphLibrary, GlyphDefinition, GlyphId}, // Added GlyphDefinition, GlyphId
};
...
impl Plugin for GamePlugin {
    fn build(&self, app: &mut App) {
        app .add_event::<UpgradeChosenEvent>() .add_event::<ItemCollectedEvent>()
            ...
            .add_systems(Update, glyph_screen_input_system.run_if(in_state(AppState::GlyphScreen)))
            .add_systems(OnExit(AppState::GlyphScreen), (despawn_ui_by_marker::<GlyphScreenUI>, on_enter_ingame_state_actions)) 
...
fn setup_glyph_screen_ui(
    ...
    .with_children(|parent| {
        ...
        // Main layout: Inventory | Socketing Areas
        parent.spawn(NodeBundle {
            style: Style {
                width: Val::Percent(100.0),
                height: Val::Percent(80.0), // Use available space
                    ...
                    if player.collected_glyphs.is_empty() {
                        ...
                        for glyph_id in player.collected_glyphs.iter() {
                            if let Some(glyph_def) = glyph_library.get_glyph_definition(*glyph_id) {
                                inventory_panel.spawn(NodeBundle {
                                    style: Style {flex_direction: FlexDirection::Column, margin: UiRect::bottom(Val::Px(5.0)), ..default()},
                                    ..default()
                                }).with_children(|glyph_entry|{
                                    glyph_entry.spawn(TextBundle::from_section(glyph_def.name.clone(), item_name_style.clone()));
                                    glyph_entry.spawn(TextBundle::from_section(glyph_def.description.clone(), item_desc_style.clone()));
                                });
                            }
                        }
                    }
            ...
            // --- Socketing Panel (Skills + Auto Weapon) ---
            main_layout.spawn(NodeBundle {
                style: Style {
                    flex_direction: FlexDirection::Column,
                ...
                socketing_panel.spawn(TextBundle::from_section("Socket Glyphs", header_style.clone()));
                ...
                if let Ok(player) = player_query.get_single() {
                    ...
                    for (skill_idx, active_skill) in player.equipped_skills.iter().enumerate() {
                        if let Some(skill_def) = skill_library.get_skill_definition(active_skill.definition_id) {
                            socketing_panel.spawn(TextBundle::from_section(format!("  {}: {}", skill_idx + 1, skill_def.name), item_name_style.clone()));
                            for (slot_idx, glyph_id_opt) in active_skill.equipped_glyphs.iter().enumerate() {
                                let slot_display_text = if let Some(gid) = glyph_id_opt {
                                    glyph_library.get_glyph_definition(*gid).map_or("Unknown Glyph".to_string(), |g_def| g_def.name.clone())
                                } else {
                                    "EMPTY SLOT".to_string()
                                };
                                socketing_panel.spawn(TextBundle::from_section(format!("    Slot {}: {}", slot_idx, slot_display_text), if glyph_id_opt.is_some() {slot_style.clone()} else {empty_slot_style.clone()}));
                            }
                        }
                    }
                    socketing_panel.spawn(NodeBundle{style: Style{height: Val::Px(10.0), ..default()}, ..default()}); // Spacer
                    ...
                    if let Some(weapon_id) = player.equipped_weapon_id {
                        if let Some(weapon_def) = weapon_library.get_weapon_definition(weapon_id) {
                            socketing_panel.spawn(TextBundle::from_section(format!("  Equipped: {}", weapon_def.name), item_name_style.clone()));
                            for (slot_idx, glyph_id_opt) in player.auto_weapon_equipped_glyphs.iter().enumerate() {
                                 let slot_display_text = if let Some(gid) = glyph_id_opt {
                                    glyph_library.get_glyph_definition(*gid).map_or("Unknown Glyph".to_string(), |g_def| g_def.name.clone())
                                } else {
                                    "EMPTY SLOT".to_string()
